...
describe("Card", () => {
  ...
  test("reconnect", async () => {
    const mockReconnect = mock.fn((mode, protocol, action) => {
    ...
    const card = new Card(mockCard);
    await card.reconnect(CardMode.SHARED, CardDisposition.RESET);
    assert.equal(mockReconnect.mock.callCount(), 1);
  });

  test("reconnectProtocol", async () => {
    const mockReconnect = mock.fn((mode, protocol, action) => {
      assert.equal(mode, CardMode.SHARED);
  ...
  test("state()", async () => {
    ...
    assert.equal(state.protocol(), mockResponse.protocol);
    assert.strictEqual(state.readerName(), mockResponse.readerName);
    assert.equal(state.status().raw, mockResponse.status);
  });
